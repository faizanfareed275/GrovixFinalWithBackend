...
try {
  ...
  io.use((socket: any, next: any) => {
    try {
      ...
      socket.auth = { userId: decoded.userId, role: decoded.role };
      return next();
    } catch {
  ...
  io.on("connection", async (socket: any) => {
    const auth = socket.auth as { userId: string; role: string } | undefined;
  ...
  console.log("[socket.io] socket.io not installed; realtime chat disabled");
}
